...
from accounts.models import student_data, userinfo
...
def uni_details(request, uni_id):
    # collect university information by using university id
    uni_info = university_list.objects.filter(Q(id=uni_id))
    store = ""
    for data in uni_info:
        store = data.uni_name  # university name

    # university all department list collect by using university name
    d_list = department_list.objects.all().filter(uni_name=store)
...
def department_deatils(request, dep_id):
    depart_details = department_list.objects.filter(
        Q(id=dep_id))  # department details information collect
