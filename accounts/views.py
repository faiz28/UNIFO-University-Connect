...
#                             request.POST['username'], request.POST['email'], request.POST['password1'], first_name=request.POST['first_name'])
#                         user.save()
#                         usersss = userinfo.objects.create(
#                             username=request.POST['username'], fullname=request.POST['first_name'])
#                         usersss.save()
#                         socialsss = student_data.objects.create(
#                             username=request.POST['username'])
#                         socialsss.save()
...
def user_profile(request):
    ...
    user_added = achievment_list.objects.filter(
...
def department_registration(request, dep_id):
    ...
    if request.method == 'POST':
        ...
        student_data.objects.filter(username=username).update(
            student_id=request.POST['stu_id'], department_id=request.POST['dep_id'], uni_name=request.POST['uni_name'], department_name=request.POST['dep_name'])
        return redirect('home')
    return render(request, 'department_registration.html', {'dep_info': depart_details, 'dep_id': dep_id})


def google(request):
    str = "http://127.0.0.1:8000/accounts/google/login/?process=login"
    return redirect(str)
